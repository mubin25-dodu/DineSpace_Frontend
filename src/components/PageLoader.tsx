"use client";

import Image from "next/image";
import { useEffect, useState } from "react";

interface param{
    load:Boolean;
    time?:number;
}

export default function (params:param){
    const [loading , setloading] = useState(params.load);
    useEffect(() => {
        if (!params.load) {
            setloading(false);
            return;
        }

        const timeout = window.setTimeout(() => setloading(false), params.time ?? 3000);
        return () => window.clearTimeout(timeout);
    }, [params.load, params.time]);

    if (!loading) return null;

    return (
        <>
        <div className="fixed inset-0 z-50 flex h-full w-full items-center justify-center bg-[#E8E4DF]">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 px-4">
             <span className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-semibold text-center">Cooking things up..</span> 
             <Image className="w-16 sm:w-20 md:w-24 lg:w-32 h-auto lg:mb-20" width={20} height={20} src="/Loadinggif.gif" alt="loading" />
            </div>
        </div>
        </>
    );
}