...
export default function Usernav(params:UsernavProps) {
    const {setActiveLink} = useContext(userContext);
    ...
    return(
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 flex h-16 flex-row items-center justify-around border-t border-[#DEC0BA] bg-[#FBF9F6] px-4 shadow-lg">
            ...
            <Link href={"/user/myBowl"} className= {`flex flex-col justify-between items-center ${pathname.startsWith('/user/myBowl') ? 'text-[#A13924]' : 'text-gray-500'}`}><Soup /> <span>My Bowl</span> </Link>
